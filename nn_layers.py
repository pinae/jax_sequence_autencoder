...
class FeedbackUnit:
    def __init__(self, input_size, latent_vector_size, params, param_init_scale=1e-2, min_feedback=0.5):
        print("Constructing a FeedpackUnit ({}, {})".format(input_size, latent_vector_size))
    ...
    def predict(self, params, input_activations):
        ...
        if self.stacked_unit:
            ...
            #print("enc factor:", (np.float32(1 - self.min_feedback) - next_layer_error_prediction)[0])
            #print("feedback factor:", (self.min_feedback + np.float32(1 - self.min_feedback) * next_layer_error_prediction)[0])
            latent_activation = (enc_activations *
                                 (np.float32(1 - self.min_feedback) - next_layer_error_prediction) +
                                 next_layer_back_projection *
                                 (self.min_feedback + np.float32(1 - self.min_feedback) * next_layer_error_prediction))
        else:
            latent_activation = enc_activations
        back_projection = self.back_projector.predict(params, latent_activation)
        error_prediction = 0#self.reconstruction_error_predictor.predict(params, latent_activation)
